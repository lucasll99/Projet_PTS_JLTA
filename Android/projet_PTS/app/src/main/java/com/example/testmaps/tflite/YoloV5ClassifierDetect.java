...
package com.example.testmaps.tflite;
...
import com.example.testmaps.DetectorActivity;
import com.example.testmaps.env.Logger;
import com.example.testmaps.env.Utils;
            ...
            for (int y = 0; y < gridWidth; ++y) {
                for (int x = 0; x < gridWidth; ++x) {
                    for (int b = 0; b < NUM_BOXES_PER_BLOCK; ++b) {
                        ...
                        final float confidence = Utils.expit(out[0][b][y * gridWidth + x][4]);
                        int detectedClass = -1;
                        float maxClass = 0;

                        final float[] classes = new float[labels.size()];
                        for (int c = 0; c < labels.size(); ++c) {
                            classes[c] = Utils.expit(out[0][b][y * gridWidth + x][5 + c]);
                        }
                        ...
                        if (confidenceInClass > getObjThresh()) {
                            final float xPos = (x + Utils.expit(out[0][b][y * gridWidth + x][0]) * 2.f - 0.5f) * (1.0f * INPUT_SIZE / gridWidth);
                            final float yPos = (y + Utils.expit(out[0][b][y * gridWidth + x][1]) * 2.f - 0.5f) * (1.0f * INPUT_SIZE / gridWidth);

                            final float w = (float) (Math.pow(Utils.expit(out[0][b][y * gridWidth + x][2]) * 2, 2) * ANCHORS[2 * MASKS[i][b]]);
                            final float h = (float) (Math.pow(Utils.expit(out[0][b][y * gridWidth + x][3]) * 2, 2) * ANCHORS[2 * MASKS[i][b] + 1]);
